...
class Server {
  ...
  async watchFiles(watchPath, watchOptions = {}) {
    ...
    for (const item of paths) {
      ...
      const scanned = picomatch.scan(pattern, { parts: true, unescape: true });

      if (scanned.negated) {
        const match = picomatch(absolute(pattern.slice(scanned.start)));
        ignored.push((file) => match(absolute(file)));
      } else if (isDynamicPattern(pattern)) {
        hasGlobs = true;
        roots.push(path.resolve(cwd, scanned.base || "."));
        matches.push(picomatch(absolute(pattern)));
      } else {
        const filePath = absolute(item);
        roots.push(path.resolve(cwd, item));
        matches.push(
    ...
    for (const item of Array.isArray(originalIgnored)
      ...
      if (typeof item === "string" && isDynamicPattern(toPosix(item))) {
        const match = picomatch(absolute(item));
        ignored.push((file) => match(absolute(file)));
      } else {
        ignored.push(item);
      }
    ...
    if (hasGlobs) {
      // Watch the glob's parent so future matches are discovered, but do not
      // register file watchers for unrelated files. Directories remain traversable.
      ignored.push(
        (file, stats) =>
          Boolean(stats?.isFile()) &&
          !matches.some((match) => match(absolute(file))),
      );
    }
